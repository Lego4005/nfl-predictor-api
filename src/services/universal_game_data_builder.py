...
class UniversalGameDataBuilder:
    ...
    def build_universal_game_data(
        ...
        try:
            ...
            # Apply comprehensive data validation and null handling
            try:
                ...
                if validation_result.fixes_applied:
                    self.logger.info(f"🔧 Applied {len(validation_result.fixes_applied)} data fixes")
                    for fix in validation_result.fixes_applied:
                        self.logger.debug(f"  • {fix}")

                if validation_result.warnings:
                    self.logger.warning(f"⚠️  {len(validation_result.warnings)} validation warnings")
                    for warning in validation_result.warnings:
                        self.logger.warning(f"  • {warning}")

                if validation_result.errors:
                    self.logger.error(f"❌ {len(validation_result.errors)} validation errors")
                ...
                self.logger.warning("⚠️  DataValidator not available, skipping validation")
