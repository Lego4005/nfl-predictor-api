...
class LocalLLMService:
    ...
    def __init__(self, base_url: str = "http://192.168.254.253:1234", timeout: int = 30):
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            success = "Connection test successful" in response.content
            if success:
                logger.info(f"✅ LLM connection test passed in {response.response_time:.3f}s")
            else:
                logger.warning(f"⚠️ LLM responded but with unexpected content: {response.content}")
