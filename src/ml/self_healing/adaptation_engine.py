...
class AdaptationEngine:
    ...
    def _apply_parameter_tuning(self, expert: Any, strategy: AdaptationStrategy) -> bool:
        ...
        try:
            # Temporarily disable adaptation to focus on core training
            logger.info(f"Adaptation temporarily disabled for {getattr(expert, 'expert_id', 'unknown')}")
            return True
    ...
    def _apply_algorithm_reset(self, expert: Any, strategy: AdaptationStrategy) -> bool:
        ...
        try:
            # Temporarily disable adaptation to focus on core training
            logger.info(f"Algorithm reset temporarily disabled for {getattr(expert, 'expert_id', 'unknown')}")
            return True
            default_traits = {
                'risk_tolerance': 0.5,
                'analytics_trust': 0.5,
