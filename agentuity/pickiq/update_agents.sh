#!/bin/bash
# Script to update remaining 3 agents with 83-bundle pattern

AGENTS=("the-rebel" "the-rider" "the-hunter")
PERSONAS=("contrarian_rebel" "momentum_rider" "value_hunter")
AGENT_IDS=("the-rebel" "the-rider" "the-hunter")
...
for i in "${!AGENTS[@]}"; do
  ...
  # Backup original
  cp "src/agents/$AGENT/index.ts" "src/agents/$AGENT/index.ts.backup"
  
  # Create updated version based on the-analyst pattern
...
export default async function ExpertAgent(
  ...
  try {
    ...
    const contextPack = await fetchContextPack('AGENT_ID_PLACEHOLDER', gameId, runId);
    ...
    let bundle = await generate83Predictions(gameContext, contextPack, ctx, 'PERSONA_PLACEHOLDER');
    ...
    if (schemaOk) {
      try {
        const storeResult = await storeExpertPredictions(runId, 'AGENT_ID_PLACEHOLDER', gameId, bundle);
    ...
    return resp.json({
      ...
      expertPersona: 'PERSONA_PLACEHOLDER',
...
AGENT_EOF
  ...
  sed -i "s/AGENT_ID_PLACEHOLDER/$AGENT_ID/g" "src/agents/$AGENT/index.ts"
  sed -i "s/PERSONA_PLACEHOLDER/$PERSONA/g" "src/agents/$AGENT/index.ts"
  
  echo "✓ $AGENT updated"
done

echo ""
echo "All agents updated! Testing compilation..."
bun run build
