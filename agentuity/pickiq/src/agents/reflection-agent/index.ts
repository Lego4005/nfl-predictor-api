...
export default async function ReflectionAgent(
  ...
  try {
    const payload = await req.data.json() as unknown as ReflectionPayload;
...
function suggestFactorAdjustments(analysis: any, expert_id: string) {
  ...
  if (analysis.incorrect_predictions.some((p: any) => p.category === 'offensive_efficiency')) {
    adjustments.push({
  ...
  if (analysis.correct_predictions.some((p: any) => p.category === 'momentum')) {
    adjustments.push({
...
function assessPredictionQuality(analysis: any) {
  return {
    best_predictions: analysis.correct_predictions.slice(0, 3).map((p: any) => p.category),
    worst_predictions: analysis.major_errors.slice(0, 3).map((p: any) => p.category),
    surprise_outcomes: analysis.close_misses.filter((p: any) => p.confidence > 0.8).map((p: any) => p.category)
  };
}

function generateMetaInsights(analysis: any, predictions: any) {
  const avgConfidence = predictions.predictions?.reduce((sum: number, p: any) => sum + p.confidence, 0) / (predictions.predictions?.length || 1);
...
function identifyBiasPatterns(analysis: any): string[] {
  ...
  if (analysis.incorrect_predictions.filter((p: any) => p.category.includes('total')).length > 2) {
    patterns.push('Systematic bias in total scoring predictions');
  }

  if (analysis.incorrect_predictions.filter((p: any) => p.category.includes('home')).length > 2) {
