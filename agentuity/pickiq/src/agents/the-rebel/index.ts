...
export default async function ExpertAgent(
  ...
  try {
    ...
    const contextPack = await fetchContextPack('the-rebel', gameId, runId);
    ...
    let bundle = await generate83Predictions(gameContext, contextPack, ctx, 'contrarian_rebel');
    ...
    if (schemaOk) {
      try {
        const storeResult = await storeExpertPredictions(runId, 'the-rebel', gameId, bundle);
