...
export default async function ExpertAgent(
  ...
  try {
    ...
    const contextPack = await fetchContextPack('the-hunter', gameId, runId);
    ...
    let bundle = await generate83Predictions(gameContext, contextPack, ctx, 'value_hunter');
    ...
    if (schemaOk) {
      try {
        const storeResult = await storeExpertPredictions(runId, 'the-hunter', gameId, bundle);
