...
def fetch_market_snap() -> Optional[List[Dict[str, Any]]]:
    if not ODDS_API_KEY:
        return None
    key = ODDS_API_KEY.strip()
    base = "/v4/sports/americanfootball_nfl/odds"
    qs = f"regions={ODDS_REGION}&oddsFormat=american&apiKey={key}"
    ...
    if not (isinstance(h2h, list) and h2h):
        return None
...
def build_su_ats_totals(games: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # placeholders for props/fantasy so UI stays consistent
...
def health():
    ...
    return {"ok": True, "version": APP_VERSION, "odds_api_key_set": bool(ODDS_API_KEY), "odds_shallow_status": status, "odds_sample_items": sample}
